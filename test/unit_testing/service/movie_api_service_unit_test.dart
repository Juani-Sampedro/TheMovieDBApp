...
void main() {
  ...
  testWidgets('throw exception if the status code is different of 200',
    ...
    await tester.pump();
    MovieListApiService apiService = MovieListApiService(client:mockClient);
    expect(
      apiService.fetch(Constants.popularMoviesEndpoint),
      throwsException,
    );
  });
  ...
  test(
    ...
    () async {
      ...
      MovieListApiService apiService = MovieListApiService(client:mockClient);
      final fetch=(await apiService.fetch(Constants.popularMoviesEndpoint)).results;
      expect(
        fetch,
        isNotEmpty,
      );
